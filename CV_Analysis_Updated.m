...
M = cell(numel(V_tank),numel(t_list));
M{numel(V_tank),numel(t_list)} =[];
...
for r = numel(V_tank)
    for i = 2:numel(t_list)
        m_tank{r,1} = m_i;
        m_tank_in(i) = m_tank{r,i-1} + m_dot_in*(t_list(i)-t_list(i-1));
        rho_tank{r,i} = m_tank_in(i)/V_tank(r);
        P_tank{r,i}= rho_tank{r,i}*R*T_i;
        [M{r,i},T,P,rho_ratio,area] = flowisentropic(gamma,(1/(rho_tank{r,i}/rho_ambient)),'dens');
            if M{r,i} > 1
                M{r,i} = 1;
                m_dot_out{r,i} = (((A_star*P_tank{r,i}))/(T_i))*sqrt(gamma/R)*(((gamma+1)/2))^((-gamma-1)/(2*(gamma-1)));
            else
                m_dot_out{r,i} = (((A_star*P_tank(i)))/(T_i))*sqrt(gamma/R)*M{r,i}*(1 + ((gamma-1)/2)*M{r,i}^2)^((-gamma-1)/(2*(gamma-1)));
            end
             m_tank{r,i}= m_tank_in(i) - m_dot_out{r,i}*(t_list(i)-t_list(i-1));
